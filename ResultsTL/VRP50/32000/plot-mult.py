...
if __name__ == "__main__":
    ...
    tl_tsp50      = pd.read_csv("TL_TNCopy/vrp-50-logs.csv")
    tl_tsp20      = pd.read_csv("VRP50_TSP20/vrp-50-logs.csv")
    ttl = ["(a) training", "(b) validation"]
    column_train = "avg_tl_epoch_train"
    column_val = "avg_tl_epoch_val"
    title = "VRP50 using TSP20 bis"
    plot_stats([ [ scratch[column_train], tl_tsp50[column_train], tl_tsp20[column_train]],
                 [ scratch[column_val], tl_tsp50[column_val], tl_tsp20[column_val]]
               ],
               ["NO-TL", "TL-TSP50", "TL-TSP20"],
               ttl,
               title,
